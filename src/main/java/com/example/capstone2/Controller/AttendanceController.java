...
public class AttendanceController {
    ...
    //Add
    @PostMapping("/add")
    public ResponseEntity addAttendance(@Valid @RequestBody Attendance attendance, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity updateAttendance(@PathVariable Integer attendanceId, @Valid @RequestBody Attendance attendance, Errors errors) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
