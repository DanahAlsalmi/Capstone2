...
public class PayrollController {
    ...
    @PostMapping("/add")
    public ResponseEntity addPayroll(@Valid @RequestBody Payroll payroll, Errors errors){
        if(errors.hasErrors()){
    ...
    public ResponseEntity updateUser(@PathVariable Integer payrollId, @Valid @RequestBody Payroll payroll, Errors errors){
        if(errors.hasErrors()){
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
